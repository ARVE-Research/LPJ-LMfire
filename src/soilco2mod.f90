...
!-------------------------------------------------------------------------------------------------------------
subroutine soilco2(co2,soil,soilprop,mtemp_air,mtemp_soil,mwet_soil,hetresp_mon, &
soilcconc_dec,soilco2conc)
...
do m = 1,12
  ...
  do l=2, nl+1, 1
      if (l .le. nl2) then
          logDgs(l) = logDg0 + log((2. * T33(1)**3.) + (0.04 * T33(1))) & 
          + ((2. + (3. / B_SHAPE)) * log(porespace(1) / T33(1)))        
      else                  
...
!      end do
      ...
      do l = 2, nl, 1    
          if (l .eq. nl) then
              soilcconc(l) = soilcconc_old(l) + dt_i * ((Dgs(l) / dz**2) * & 
              (soilcconc_old(l-1) - soilcconc_old(l)) + hetresp_mon_layers(m, l))
          else                   
               soilcconc(l) = soilcconc_old(l) + dt_i * ((Dgs(l) / dz**2) * & 
               (soilcconc_old(l+1) - 2 * soilcconc_old(l) + soilcconc_old(l-1)) + & 
               ((Dgs(l+1) - Dgs(l-1)) * (soilcconc_old(l+1) - soilcconc_old(l-1))) / & 
               (4 * dz**2) + hetresp_mon_layers(m, l))
  ...
  soilco2conc(m) = (sum(soilcconc(2:nl)) * 8.3143 * (T0 + mtemp_soil(m)) &
  / (44.01 / 1000. * 101325.)) / nl
...
!write(stdout,*)'soilco2conc',soilco2conc
