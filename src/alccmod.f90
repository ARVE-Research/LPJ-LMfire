...
subroutine alcc(j,input,osv,agriculture,unusable,coverfrac,recoverf)
...
real(sp),                intent(inout) :: unusable    ! urban land
real(sp),                intent(inout) :: agriculture ! cropland + pasture + rangeland 
...
agriculture = min(max(0.,agriculture),1.) ! make sure agriculture is between 0 and 1
...
lndturn = tf * agriculture
usable  = 1. - unusable

if (agriculture > usable) then   ! this should only happen during the interpolation between 1850 and present-day
  usable   = agriculture
  unusable = 1. - usable
end if
...
convf = min(agriculture - coverfrac(2) + lndturn,usable)  

coverfrac(2) = agriculture
...
agriculture       => osv%tile(2)%coverfrac
