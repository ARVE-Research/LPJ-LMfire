...
use parametersmod,    only : sp,dp,npft,stdout,stderr
...
type(sizeind), dimension(:,:), intent(in)    :: ind           ! size of the individuals by PFT and height class
...
real(sp), parameter :: wmin = 0.001  ! moisture content threshold below which we zero out fuel moisture to avoid underflow
...
real(sp) :: cont_area  ! average contiguous area (ha)
...
! write(0,*)wfinefuel,wlivegrass,SOM_surf,omega_lg,omega(1) 
...
! 2023.03 had to move this down below the first calculation of FDI
...
!  write(*,'(a,2i4,6f14.7)') 'tree-dominated_ROS ', year,d, ROSfsurface, Uforward, rho_b, sigma, relmoist, wn
  ...
  if (treecover >= 0.6) then
    ...
    canopywater = sum(wscal * fpc_grid,mask=pft%tree .and. fpc_grid > 0.) / treecover
