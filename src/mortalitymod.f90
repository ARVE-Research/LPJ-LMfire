...
subroutine mortality(pftpar,present,tree,boreal,bm_inc,              &
       ...
       litter_ag_fast,litter_ag_slow,litter_bg,dtemp,anpp,mtemp_max,cellarea)

! arguments

integer,                  intent(in)    :: year
real(sp),                 intent(in)    :: cellarea
...
real(sp) :: nind_min   ! tree density is set to a minimum of one individual per gridcell

! ---------------------------------------------------------
! this stress-related mortality only affects woody plants

nind_min = 1.e-10 ! this is the value set in establishmentmod

do pft = 1,npft

  twmax=pftpar(pft,30)  ! PFT-specific upper limit of warmest-month temperature

  if (present(pft) .and. tree(pft) .and. nind(pft) >= nind_min) then 
