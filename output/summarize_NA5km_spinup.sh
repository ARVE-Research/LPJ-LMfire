...
# take the first tavg years of the simulation and use that for the summary

# ---

tavg=180

timeavg=${infile%%.*}_spinup-avg_$tavg.nc

echo "make time average" $tavg

ncra -F -O -v cover,burnedf,acflux_fire -d time,,$tavg $infile $timeavg
